...
if (file_exists($logFile)) {
    ...
    if ($offset >= $totalLines) {
        echo json_encode(['data' => '', 'endOfFile' => true]);
        exit;
    }
    ...
    // JSON 응답 전송 (로그 데이터와 endOfFile 플래그 전송)
    echo json_encode(['data' => nl2br(implode("", $logLines)), 'endOfFile' => false]);
} else {
    // 파일이 없을 경우 JSON으로 오류 메시지 전송
    echo json_encode(['data' => 'nohup.out 파일을 찾을 수 없습니다.', 'endOfFile' => true]);
}
?>
